...
export class FacebookService {
  ...
  private async handleIncomingMessage(accountId: string, event: any, platform: 'facebook' | 'instagram'): Promise<void> {
    ...
    // Resolve best access token for profile lookup
    let tokenToUse = account?.accessToken as string | undefined;
    if (platform === 'instagram' && (!tokenToUse || tokenToUse.length < 10)) {
      try {
        const fbAccount = await this.socialAccountModel.findOne({
          tenantId: new Types.ObjectId(tenantId),
          platform: SocialPlatform.FACEBOOK,
          pageId: account?.pageId || recipientId || accountId,
          status: SocialAccountStatus.CONNECTED,
        }).lean();
        if (fbAccount?.accessToken) {
          tokenToUse = fbAccount.accessToken;
          this.logger.log('[handleIncomingMessage] Using linked Facebook Page access token for Instagram profile lookup');
        } else {
          this.logger.warn('[handleIncomingMessage] No access token found on linked Facebook Page for Instagram profile lookup');
        }
      } catch (e) {
        this.logger.warn(`[handleIncomingMessage] Failed to resolve FB page token for IG lookup: ${e instanceof Error ? e.message : e}`);
      }
    }
    const contact = await this.getOrCreateContact(tenantId, senderId, tokenToUse, platform);
  ...
  private async getOrCreateContact(
    ...
    // Get profile from Facebook/Instagram to update name even if contact exists
    ...
    if (pageAccessToken) {
      try {
        // Instagram: use username, name, profile_pic (IGSID)
        // Messenger: use first_name, last_name, profile_pic (PSID)
        const fields = platform === 'instagram' 
          ? 'id,username,name,profile_pic' 
          : 'id,first_name,last_name,profile_pic';
        const url = `${this.graphApiUrl}/${senderId}?fields=${fields}&access_token=${pageAccessToken}`;
        this.logger.log(`[getOrCreateContact] Fetching ${platform} profile from: ${url}`);
        this.logger.log(`[getOrCreateContact] Using access token (first 20 chars): ${pageAccessToken.substring(0, 20)}...`);
        const res = await fetch(url);
        const profile = await res.json() as any;
        
        this.logger.log(`[getOrCreateContact] ${platform} API response: ${JSON.stringify(profile)}`);
        
        if (profile.error) {
          this.logger.error(`[getOrCreateContact] ${platform} API error: ${profile.error.message} (type: ${profile.error.type})`);
        }
        
        if (platform === 'instagram') {
          // Instagram may have 'name' (full name) or 'username' (handle)
          if (profile.name) {
            name = profile.name;
            this.logger.log(`[getOrCreateContact] Instagram name from 'name' field: ${name}`);
          } else if (profile.username) {
            name = profile.username;
            this.logger.log(`[getOrCreateContact] Instagram name from 'username' field: ${name}`);
          } else {
            this.logger.warn(`[getOrCreateContact] Instagram profile has no 'name' or 'username' field`);
          }
          profilePic = profile.profile_pic;
          
          // Store username for socialProfiles
          if (profile.username) {
            username = profile.username;
            this.logger.log(`[getOrCreateContact] Instagram username: ${username}`);
          }
        } else {
          // Messenger uses first_name and last_name
          if (profile.first_name) {
            name = `${profile.first_name}${profile.last_name ? ' ' + profile.last_name : ''}`;
            this.logger.log(`[getOrCreateContact] Messenger name: ${name}`);
          } else {
            this.logger.warn(`[getOrCreateContact] Messenger profile has no 'first_name' field`);
          }
          profilePic = profile.profile_pic;
        }
        this.logger.log(`[getOrCreateContact] ${platform} profile fetched: name="${name}", hasProfilePic=${!!profilePic}`);
      } catch (e) {
        this.logger.warn(`Failed to get ${platform} profile for ${senderId}: ${e.message}`);
      }
    } else {
      this.logger.warn(`[getOrCreateContact] No pageAccessToken provided for ${platform}`);
    }

    if (contact) {
      // Update existing contact with latest name, profile pic, and username
      if (contact.name !== name || contact.avatar !== profilePic) {
        this.logger.log(`[getOrCreateContact] Updating contact ${contact._id}: name "${contact.name}" -> "${name}", avatar updated: ${!!profilePic}`);
        contact.name = name;
        if (profilePic) contact.avatar = profilePic;
        if (username && platform === 'instagram') {
          contact.socialProfiles = contact.socialProfiles || {};
          contact.socialProfiles.instagram = contact.socialProfiles.instagram || { id: senderId };
          contact.socialProfiles.instagram.username = username;
          this.logger.log(`[getOrCreateContact] Updated Instagram username: ${username}`);
        }
        await contact.save();
      }
      return contact;
    ...
    const socialProfiles = platform === 'instagram' && username
      ? { instagram: { id: senderId, username } }
      : undefined;

    this.logger.log(`[getOrCreateContact] Creating new contact: name="${name}", platform=${platform}, senderId=${senderId}`);
